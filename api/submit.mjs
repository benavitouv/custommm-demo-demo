...
const uploadAttachment = async (file) => {
  ...
  const filename = file.name || 'vendor-contract-upload';
...
const triggerWebhook = async ({ vendorName, vendorId, vendorType, attachmentId }) => {
  const webhookResponse = await fetch(WEBHOOK_URL, {
    ...
    body: JSON.stringify({
      ...
      payload: {
        ...
        contract_attachment_id: attachmentId,
      },
    }),
  });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const vendorType = String(formData.get('vendor_type') || '').trim();
    const file = formData.get('contract');

    if (!vendorName || !vendorId || !vendorType) {
    ...
    if (!preUploadedId && (!file || typeof file === 'string')) {
      return jsonResponse(res, 400, {
        ok: false,
        error: 'missing_file',
        message: 'A contract document attachment is required.',
      });
    }
